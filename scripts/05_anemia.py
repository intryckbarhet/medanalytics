...
# выкидываем тех, у кого нет метки анемии (на чем нам учиться-то?)
df_model = df.dropna(subset=['target_anemia']).copy()

X = df_model[features]
y = df_model['target_anemia'].astype(int)
...
model.save_model('models/anemia_model.cbm')
print("модель сохранена в models/anemia_model.cbm")
